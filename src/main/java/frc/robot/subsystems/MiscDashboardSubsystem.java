...
public class MiscDashboardSubsystem extends SubsystemBase {
    ...
    public MiscDashboardSubsystem() {
        ...
        withSize(2,2).withWidget(BuiltInWidgets.kDial).withProperties(Map.of("min", 0, "max", 1000)); //Disclaimer: I DON'T KNOW THE ACTUAL TOP SPEED.

        main_tab.addNumber("Midtake Motor Speed (RPM)", () -> midSpeed).withPosition(2, 2).
        withSize(2,2).withWidget(BuiltInWidgets.kDial).withProperties(Map.of("min", 0, "max", 1000)); //Disclaimer: I DON'T KNOW THE ACTUAL TOP SPEED.

        main_tab.addNumber("Intake Motor Current (A)", () -> inCurrent).withPosition(4, 2).
        withSize(2,2).withWidget(BuiltInWidgets.kDial).withProperties(Map.of("min", 0, "max", 600));

        main_tab.addNumber("Midtake Motor Current (A)", () -> midCurrent).withPosition(5, 0).
        withSize(2,2).withWidget(BuiltInWidgets.kDial).withProperties(Map.of("min", 0, "max", 600));
